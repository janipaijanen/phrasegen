...
#
# Copyright (C) 2012-2015 Jani Päijänen
...
logging.basicConfig(format='%(asctime)s %(filename)s %(lineno)04d  %(message)s')
...
try:
  ...
  sys.stderr.write( "install BeautifulSoup")
  logger.fatal("install BeautifulSoup")
  sys.exit(1)
import time 

class MyOpener (urllib.FancyURLopener):
  version = 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2.15) Gecko/20131019 phrasegen/1.1.0'


Newlines = re.compile (r'[\r\n]\s+')

encoding_guess_list=['utf8', 'cp775', 'cp1250', 'iso-8859-1', 'iso-8859-15', 'iso-8859-13', 'iso-8859-16', 'latin1', ]
def try_unicode2(string, errors='strict'):
...
def stilize_page(text):
  try:
    bs = BeautifulSoup(text, convertEntities=BeautifulSoup.HTML_ENTITIES)# , smartQuotesTo=None) #,fromEncoding="utf-8")
    #)
    ...
    if (bs.find('body') != None):
      logger.debug (bs.originalEncoding)
  ...
  for r in result:
    ...
    if i >=wordcount:
      ...
      if replace_spaces is not None:
        ...
        mychr = unicode("").join(replace_spaces[random.randint(0,re_len)])
        logger.debug('"' + mychr + '"')
...
def main (default_urls=False):
  ...
  parser.add_option ( "--replace-spaces", dest="replace_spaces", default=None,
                    action="store", type="string", help="Allow spaces in text. If set to false, then spaces are omitted. Default=True")
  ...
  if options.default_urls == True:
    urls_list = []
    urls_list.append ("http://fi.wikipedia.org/wiki/Special:Random")
    ...
    urls_list.append ("http://se.wikipedia.org/wiki/Special:Random")
    urls_list.append ("http://de.wikipedia.org/wiki/Special:Random")
    urls_list.append ("http://en.wikipedia.org/wiki/Special:Random")
  ...
  #logger.debug(options.replace_spaces)
  ...
  do_writefile(data, options.fout, options.wordscount, options.replace_spaces)
